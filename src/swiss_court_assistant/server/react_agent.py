...
ASK_TOOL = """- ask_user(question, options, found_so_far): ask the user one short question instead of answering, when the answer turns on a fact the question leaves open and the passages found go different ways on it — a residential or a commercial lease, which canton, whether a deadline has passed, employee or self-employed. Ask about one fact only, the one the answer turns on most, in one sentence, in the language of the user's question (not the language you searched in). options are two to four short answers to that question, in the user's language; no \"other\" option, the user can always type their own. found_so_far says in two or three sentences what the research found and which decisions (decision_id) matter for each case; the next turn reads it back. Never settle the open fact yourself: if answering means assuming one case — that the contract is an employment contract, that the user is the tenant — ask instead. Ask only after searching, when you can say why the answer differs; never for something the tools can look up, and not when a short answer can cover every case.
"""
...
def make_tools(corpus: Corpus) -> list:
    ...
    async def ask_user(question: str, options: list[str], found_so_far: str) -> str:
        ...
        question leaves open. options: two to four short likely answers. found_so_far: what the research
        found so far and which decision_ids matter, for the next turn."""
